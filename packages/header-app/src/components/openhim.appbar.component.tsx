...
export default function OpenhimAppBar() {
  ...
  const OpenhimLogo = () => (
    <img
      alt="OpenHIM Console"
      style={{textDecoration: 'none', width: '100px', height: '40px', marginBottom: isLoggedIn ? '0px' : '8px'}}
        ...
        <Toolbar disableGutters>
          {isLoggedIn && isAdmin && (
            <Box sx={{flexGrow: 1, display: {xs: 'flex', md: 'none'}}}>
              ...
              <Typography
                sx={{marginLeft: 1, display: {xs: 'inline-block', md: 'none', lg: 'none'}}}
                variant="body1"
                noWrap
            ...
            <Box sx={{flexGrow: 0}}>
              <Tooltip title="Open settings">
                <IconButton onClick={handleOpenUserMenu} sx={{p: 0}}>
                  <Person style={{width: 26, height: 26}} />
                </IconButton>
              </Tooltip>
              <Menu
                sx={{mt: '45px'}}
