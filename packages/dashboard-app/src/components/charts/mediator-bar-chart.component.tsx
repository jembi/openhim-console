...
export default function MediatorBarChart(props: MediatorBarChartProps) {
  ...
  const formattedTimestamps = timestamps.map(timestamp => format(timestamp, 'yyyy-MM-dd HH:mm:ss'));
  ...
  return (
    <BarChart
      xAxis={[{ scaleType: 'band', data: Object.entries(counts).map(([timestamp, count]) => timestamp) }]}
      series={chartData}
      width={500}
      height={300}
    />
