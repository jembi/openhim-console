...
export default function TransactionLineChart(props: TransactionLineChartProps) {
  ...
  const buildChartData = () => {
    const graphData = []
    const { diff, scale } = getTimeDiffScale(props.period.from, props.period.until);
    ...
    for (let i = 0; i < diff; i++) {
      ...
      for (let j = 0; j < props.data.length; j++) {
        ...
        if (eq) {
          Y = props.data[j][props.type == 'avgResponseTime' ? 'avgResp' : 'total'];
          break;
        }
      }
  ...
  return (
    <Card style={{ minWidth: '600px', }}>
      <CardHeader
        title={props.type == 'load' ? "Transaction Load" : "Average Response Time"}
      ...
      <CardContent>
        ...
        {hasDataToShow && <LineChart
          ...
          yAxis={[{ label: props.type == 'load' ? 'Load' : 'Response Time' }]}
          series={[
            {
              data: yData,
              area: true,
