export function channelRequestMatchingCtrl($scope, Api) {
  ...
  Api.Clients.query(
    ...
    function () {
      /* server error - could not connect to API to get clients */
    }
  )
  ...
  if ($scope.update) {
    $scope.channel.$promise.then(function () {
      if ($scope.channel.matchContentRegex) {
        $scope.matching.contentMatching = 'RegEx matching'
      }
      if ($scope.channel.matchContentJson) {
        $scope.matching.contentMatching = 'JSON matching'
      }
      ...
      if (
        $scope.channel.matchContentRegex ||
        $scope.channel.matchContentJson ||
