...
export function ChannelMonitoringCtrl(
  $scope,
  $uibModal,
  $interval,
  $location,
  ...
  const noDataErrorMsg =
    'There has been no transactions received for the queried timeframe'
  ...
  function loadMetricsSuccess(metrics) {
    if (metrics.length === 0) {
      ...
      $scope.transactionLoadData = Metrics.buildLineChartData(
        $scope.selectedDateType,
        metrics,
        'total',
        'Transactions'
      )
      $scope.transactionResponseTimeData = Metrics.buildLineChartData(
        $scope.selectedDateType,
        metrics,
        'avgResp',
        'Response Time (ms)',
  ...
  function loadMetricsError(err) {
    ...
    Alerting.AlertAddMsg(
      'load',
      'danger',
      'Transaction Load Error: ' + err.status + ' ' + err.data
    )
    Alerting.AlertAddMsg(
      'responseTime',
      'danger',
  ...
  const updateChannelsBarChart = function (channelsData) {
    ...
    for (let i = 0; i < channelsData.length; i++) {
      channelsBarData.push({
        label: channelsData[i].label,
        value: channelsData[i].value
      })
    }
    $scope.channelsBarData = {
  ...
  const updateChannelsDonutChart = function (channelsData) {
    ...
    for (let i = 0; i < channelsData.length; i++) {
      channelsDonutData.push({
        label: channelsData[i].label,
        value: channelsData[i].percent
      })
      channelsDonutColors.push(channelsData[i].color)
    }
    $scope.channelsDonutData = {
      data: channelsDonutData,
      colors: channelsDonutColors
    }
  }
  ...
  const channelsMetricsError = function (err) {
    ...
    Alerting.AlertAddMsg(
      'status',
      'danger',
      'Transaction Channels Error: ' + err.status + ' ' + err.data
    )
  }
