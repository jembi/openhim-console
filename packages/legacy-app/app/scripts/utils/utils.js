...
export function isBase64String(string) {
  return /^(?:[A-Za-z0-9+/]{4})*(?:[A-Za-z0-9+/]{2}==|[A-Za-z0-9+/]{3}=|[A-Za-z0-9+/]{4})([=]{1,2})?$/.test(
    string
  )
}
...
export function returnContentType(objectHeaders) {
  ...
  const [contentKey = 'Content-Type'] = Object.keys(objectHeaders).filter(k =>
...
export function beautifyIndent(type, content) {
  try {
    ...
    if (
      ...
      type.indexOf('application/html') >= 0
    ) {
