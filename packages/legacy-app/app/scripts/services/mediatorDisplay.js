...
export function MediatorDisplay(config) {
  const _formatMediator = function (mediator) {
    ...
    if (!mediator._lastHeartbeat) {
      ...
      mediator.lastHeartbeatDisplayExplain =
        "No heartbeats have ever been received from this mediator. Perhaps it doesn't support heartbeats."
    } else {
      if (moment(mediator._lastHeartbeat).isAfter(Date.now())) {
        ...
        secondsDiffNow() < config.mediatorLastHeartbeatWarningSeconds
      ) {
        mediator.lastHeartbeatStatus = 'success'
      } else if (secondsDiffNow() < config.mediatorLastHeartbeatDangerSeconds) {
        mediator.lastHeartbeatStatus = 'warning'
        mediator.lastHeartbeatDisplayExplain =
          'No heartbeats received in over ' +
          config.mediatorLastHeartbeatWarningSeconds +
          ' s'
      } else {
        mediator.lastHeartbeatStatus = 'danger'
        mediator.lastHeartbeatDisplayExplain =
          'No heartbeats received in over ' +
          config.mediatorLastHeartbeatDangerSeconds +
          ' s'
      }
    ...
    if (mediator._uptime) {
      ...
      mediator.uptimeDisplay = moment()
        .subtract(mediator._uptime, 'seconds')
        .fromNow(true)
      mediator.uptimeSince = moment(mediator._lastHeartbeat)
        .subtract(mediator._uptime, 'seconds')
        .toDate()
    }
  }
