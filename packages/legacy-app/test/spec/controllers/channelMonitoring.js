...
describe('Controller: ChannelMonitoringCtrl', function () {
  ...
  beforeEach(inject(function ($controller, $rootScope, $httpBackend) {
    ...
    var timeLoadData = [
      {total: 34, avgResp: 2881.91, timestamp: daysAgo(6).format()},
    ...
    $httpBackend
      .when(
        'GET',
        new RegExp(
          '.*/metrics/timeseries/hour/channels/5322fe9d8b6add4b2b059dd8'
        )
      )
      .respond([])
    $httpBackend
      .when(
        'GET',
        new RegExp(
          '.*/metrics/timeseries/day/channels/5322fe9d8b6add4b2b059dd8'
        )
      )
      .respond(timeLoadData)
    $httpBackend
      .when('GET', new RegExp('.*/metrics/channels/5322fe9d8b6add4b2b059dd8'))
      .respond(channelsData)
    $httpBackend
      .when('GET', new RegExp('.*/channels/5322fe9d8b6add4b2b059dd8'))
      .respond({
    ...
    createController = function () {
      ...
      scope = $rootScope.$new()
      return $controller('ChannelMonitoringCtrl', {
        $scope: scope,
        $routeParams: {channelId: '5322fe9d8b6add4b2b059dd8'}
      })
    }
  }))
  ...
  it('should run updateMetrics() and set the transactionTimeData graph object', function () {
    httpBackend.expectGET(
      new RegExp('.*/metrics/channels/5322fe9d8b6add4b2b059dd8')
    )
    createController()
    scope.selectedDateType.from = daysAgo(6).toDate()
    scope.selectedDateType.until = daysAgo(0).toDate()
    ...
    scope.transactionResponseTimeData.data[5].should.have.property(
      'value',
      '4564.57'
    )
    scope.transactionResponseTimeData.data[6].should.have.property('timestamp')
    scope.transactionResponseTimeData.data[6].should.have.property(
      'value',
      '3553.34'
    )
