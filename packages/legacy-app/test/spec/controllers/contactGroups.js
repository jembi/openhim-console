...
describe('Controller: ContactGroupsCtrl', function () {
  ...
  beforeEach(inject(function (
    ...
    $httpBackend.when('GET', new RegExp('.*/groups')).respond([
      ...
      {
        group: 'Group 2',
        users: [{user: 'User 22', method: 'email', maxAlerts: 'no max'}]
      },
      {
        group: 'Group 3',
        users: [{user: 'User 333', method: 'email', maxAlerts: '1 per hour'}]
      },
      {
        group: 'Group 4',
        users: [{user: 'User 4444', method: 'sms', maxAlerts: '1 per day'}]
      }
    ])
